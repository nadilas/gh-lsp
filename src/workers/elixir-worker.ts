/**
 * Web Worker entry point for Elixir language support.
 *
 * Loaded by the WorkerPool when an Elixir file is opened.
 * Routes JSON-RPC messages between the background service worker and the
 * Elixir code analyzer adapter.
 */

import { LspWorkerHost } from './lsp-worker-host';
import { createElixirServer } from './servers/elixir-server';
